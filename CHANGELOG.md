...
Production-refactor batch (Phases 3â€“4): frontend entitlement gating + backend hardening.
...
### Changed / Fixed
- `SqlGuard` now rejects subqueries/CTEs, guaranteeing the fallback NLâ†’SQL path is a single flat
...
Production-refactor batch (Phases 0â€“2). All changes are backwards compatible and additive;
...
- Safe AI analytics query pipeline (`ai.query`): the LLM emits a structured, validated query intent
  ...
  to the existing SqlGuard NLâ†’SQL path when no valid intent is produced, so no analytics is lost.
...
- Alert types: `BUDGET_WARNING` (â‰¥80% of budget spent), `BUDGET_EXCEEDED` (â‰¥100%), `SPENDING_SPIKE` (current month > previous month Ã— 1.5)
...
- Expenses page: **Export CSV** button in the filter toolbar; passes active `from`/`to` filter so "what you see is what you download"; shows "Exportingâ€¦" spinner while download is in progress
...
- Demo seed data: 54 synthetic expenses spanning Janâ€“Jun 2026 across all 13 predefined categories; 5 sample budgets for June 2026; 6 recurring expenses seeded on first startup when `GASTOS_SEED_SAMPLE_DATA=true`
...
- `GET /ai/insights/recommendations?month=YYYY-MM` â€” returns 2â€“3 AI-generated actionable spending recommendations based on category and MoM data
