...
Production-refactor batch (Phases 3Ã¢â‚¬â€œ4): frontend entitlement gating + backend hardening.
...
### Changed / Fixed
- `SqlGuard` now rejects subqueries/CTEs, guaranteeing the fallback NLÃ¢â€ â€™SQL path is a single flat
...
Production-refactor batch (Phases 0Ã¢â‚¬â€œ2). All changes are backwards compatible and additive;
...
- Safe AI analytics query pipeline (`ai.query`): the LLM emits a structured, validated query intent
  ...
  to the existing SqlGuard NLÃ¢â€ â€™SQL path when no valid intent is produced, so no analytics is lost.
...
- Alert types: `BUDGET_WARNING` (Ã¢â€°Â¥80% of budget spent), `BUDGET_EXCEEDED` (Ã¢â€°Â¥100%), `SPENDING_SPIKE` (current month > previous month Ãƒâ€” 1.5)
...
- Expenses page: **Export CSV** button in the filter toolbar; passes active `from`/`to` filter so "what you see is what you download"; shows "ExportingÃ¢â‚¬Â¦" spinner while download is in progress
...
- Demo seed data: 54 synthetic expenses spanning JanÃ¢â‚¬â€œJun 2026 across all 13 predefined categories; 5 sample budgets for June 2026; 6 recurring expenses seeded on first startup when `GASTOS_SEED_SAMPLE_DATA=true`
...
- `GET /ai/insights/recommendations?month=YYYY-MM` Ã¢â‚¬â€ returns 2Ã¢â‚¬â€œ3 AI-generated actionable spending recommendations based on category and MoM data
