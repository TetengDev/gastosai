...
### Added
- **Recurring budgets.** A budget can be marked "Repeat every month" (checkbox in the add/edit modal); it is then **carried forward automatically** to later months â€” when you open a month with no budget for that category, the most recent recurring limit is materialized for it (currency/rate preserved). Turning recurring off on a later month stops the carry-forward. Recurring budgets show a **â†» Monthly** badge. (`Budget.recurring`, Flyway V18; `BudgetRequest`/`BudgetResponse` gain `recurring`.)
- **Rule-based budgeting (50-30-20 and presets).** Users can follow a budgeting technique that splits monthly income across **Needs / Wants / Savings** buckets. Presets: 50/30/20, 70/20/10, and a **Custom** split (validated to sum to 100). Categories are tagged to a bucket, and a summary shows the per-bucket **target (income Ã— %) vs actual spend** for the month, with the leftover from unassigned categories surfaced separately.
  - New `budget_rules` table + `categories.bucket` column (Flyway **V16**); `BudgetRule`/`Bucket`/`BudgetRuleType` entities.
  - Endpoints: `GET/PUT /budget-rules`, `PUT /budget-rules/buckets` (bulk categoryâ†’bucket assignment), `GET /budget-rules/summary?month=`.
  ...
  - **Opt-in:** the feature is off by default â€” the Budget page shows a dismissible "try it" invite until the user enables it (`PUT /budget-rules/enabled`, Flyway V17 `enabled` flag), and a **Hide** control turns it back off.
...
### Added
- **Tiered chat tones.** The professional and Gen Z chat tones are now a premium capability (`CHAT_PERSONAS`); the plain tone stays free. When monetization is enforced, a non-entitled user's request for a premium tone gracefully **falls back to plain** instead of failing. The chat widget shows a Ã°Å¸â€â€™ on the locked tones. New `FeatureKey.CHAT_PERSONAS` granted to PREMIUM + TRIAL.
- **Per-plan category caps.** Creating categories beyond the plan limit is blocked when monetization is enforced Ã¢â‚¬â€ FREE is capped at 5 (`gastos.limits.categories.{free,premium,trial}`, `0` = unlimited); returns HTTP 402 with an upgrade message. Admin/PREMIUM are effectively unlimited.
...
- Monetization stays **off by default** (`MONETIZATION_ENFORCE=false`) Ã¢â‚¬â€ every feature unlocked for everyone during alpha/beta; the boundaries above only apply once enforcement is flipped on.
...
### Changed
- **Dashboard layout ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â removed blank spaces.** The spending-vs-budget row no longer stretches the shorter "Budget overview" panel into a tall empty box (`items-start`), and the forward-looking row now goes straight from 1 to 3 columns (`md:grid-cols-3`) so the third card is never orphaned on tablet. Cards keep their natural height instead of leaving trailing voids.
- **First-run tour refreshed.** Replaced the hardcoded indigo/light-only styling with the app's theme tokens (brand green primary, surface/ink colors resolved at runtime) so the tour now matches **both light and dark mode**. Rewrote the steps to cover current key features ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â flexible expense logging, budgets & safe-to-spend, recurring bills, savings goals, the AI assistant (tone modes + chat history + receipt scanning), and Settings/AI key.
...
### Security
- Type-mismatch request errors (e.g. a non-numeric `page`/`size`) now return a generic 400 ("Invalid value for parameter 'x'.") instead of echoing the raw rejected input ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â closes the low-severity reflected-input note from the security review.
...
### Added
- **Chat audit log (Phase 2b, slice 3 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â completes Phase 2b).** Every chatbot tool invocation is now recorded to a `chat_audit_logs` table (Flyway V15): user, conversation, tool name, outcome (SUCCESS/FAILED), and a short redacted detail (e.g. "preview", error class). Provides a per-user security/debugging trail for the assistant. Writing is best-effort ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â an audit failure never breaks a chat reply.
...
### Added
- **Chat follow-up context resolution (Phase 2b, slice 2).** Within a conversation the assistant now resolves follow-ups like "delete it" / "make it 500" / "that one". Hybrid approach: the last expense a turn creates/updates is remembered on the conversation (`last_entity_type`/`last_entity_id`, Flyway V14), and each new message is prefixed with a short recent transcript + last-entity hint before intent classification ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â so the model can resolve the referent. Deterministic capture is unit/integration-tested; the LLM resolution quality is verified via the chat E2E + manual use.

### Security
- Error responses no longer include stack traces (`server.error.include-stacktrace=never`) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â closes an app-wide info-leak on unmatched/unhandled routes surfaced by the Phase 2b security review.

### Tests
- Added a Playwright E2E (`e2e/chat-conversation.spec.ts`) covering the chat History drawer (seed ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ list ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ reopen ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ new ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ delete).
...
### Changed
- `AiRateLimitInterceptor` and `PublicRateLimitInterceptor` no longer keep their own counters ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â both use the shared `RateLimiterStore` (behavior identical in the default in-memory mode).
...
### Security
- **NLÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢SQL fallback tenant-isolation hardening (F-06)** from the authorized penetration test:
  - The guarded NLÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢SQL fallback query now runs with a 5-second statement timeout (`GuardedFallbackExecutor`), matching the structured analytics path so neither AI database path can run unbounded.
  ...
  - Documented the `SqlGuard` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â `appendUserFilter` coupling in code and in `ai/skills/ai-sql-safety.md`: relaxing SqlGuard's single-SELECT/subquery rules requires a paired review and an extension of the isolation test.
...
### Fixed
- **Production startup crash on the per-user categories migration (Flyway V11).** V11 cloned each predefined category per user before dropping the old global unique-on-name constraint (`categories_name_key`), so the first duplicate category name collided and the migration aborted ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the backend failed to boot. The constraint is now dropped before the clone step. Note: a previously failed V11 leaves a `success=false` row in `flyway_schema_history` that must be cleared (`DELETE FROM flyway_schema_history WHERE version = '11' AND success = false;`) before redeploying.
...
### Security
- **Categories are now per-user (multi-tenant isolation).** Previously the category list was global, so any signed-in user could view, rename, re-icon, or delete every user's categories ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â and deleting one reassigned other users' expenses. Categories are now owned per user and every category operation (REST + chatbot tools) is scoped to the authenticated user; budgets can only attach to your own categories. Fixes pentest finding F-01.
...
### Added
- Passwordless **email magic-link login** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â enter your email, get a one-time sign-in link, click it to log in. Works alongside the existing email+password login. First-time emails get a passwordless account automatically. Endpoints: `POST /auth/magic-link`, `POST /auth/magic-link/verify`. Tokens are single-use, hashed at rest, and expire in 15 minutes.
...
### Changed
- PREMIUM plan price set to ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±149 (from ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±199); the seeder now reconciles the price on existing databases.
...
### Added
- Contact and Feedback pages ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â public (no login) forms to send a message or a suggestion. Submissions are stored and shown to admins in a new in-app "Messages" view (`/admin/submissions`). Backed by a new `submissions` table (Flyway V8); `POST /submissions` is public, `GET /submissions` is admin-only.
...
- Category icons now appear wherever a category is shown ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Expenses table, dashboard (Spending by Category, Recent Expenses, Top Expenses), Upcoming Bills, Budget, and Recurring ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â via a shared `categoryIcon` helper + `CategoryChip`.
...
### Changed
- Dashboard reordered for clearer hierarchy (ui-ux-reviewed): hero ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Daily Trend + **Recent Expenses** side by side ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ AI Insights ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Spending by Category + Budget ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Alerts + Top Expenses ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Goals + Upcoming Bills ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Monthly Trend. Monthly-Trend axis abbreviates (ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±12.5k); Alerts empty state reassures.

### Changed
- Login and Register screens restyled to the design system (tokens, brand wordmark, primitives) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â they no longer use the old indigo theme.
...
### Added
- Structured JSON logging in production (Spring Boot native ECS format to stdout) for ingestion by a hosted aggregator ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â dev keeps human-readable console logs.
- Per-request correlation id + access log: `RequestLoggingFilter` assigns/propagates an `X-Request-Id` (echoed in the response header) and logs one `http_request` line per request with method, path, status, duration, and user ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â no secrets.
...
### Changed
- Full visual redesign applied across every screen ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Dashboard (deep-green hero + stat grid, daily-trend & category bars, budget/alerts), Expenses, Categories, Budget, Recurring, Goals, Settings, and the Chat widget ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â using the new tokens, type scale, and shared primitives.
...
- Tour steps reordered to match the navbar (Expenses ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Budget ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Recurring ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Goals) and restyled (spacing, spotlight, overlay).
...
- Admin-only **"View As"** switcher in the navbar: preview the app as a Free or Premium tier, with AI on/off, and as a regular user ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â without changing real data or keys. Sends `X-View-As-*` headers honored only for admins; `EntitlementService`/AI gate simulate the chosen state, and the UI (FeatureGate, insights, chat) updates live.
...
### Added
- Server-side caching (Caffeine) for AI insight responses (top-category, month-summary, recommendations), keyed per user + month. Repeat dashboard loads within the TTL skip the LLM call ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â faster and fewer tokens spent on the user's key. Cache is evicted on any expense create/update/delete. Tunable via `gastos.insights.cache.{enabled,ttl-minutes,max-size}` (`INSIGHTS_CACHE_*` env), default 15-min TTL.
...
### Fixed
- AI NL-query fallback is now always user-scoped, including admins ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â admins previously ran unscoped raw AI SQL (Phase 1 audit HIGH). Cross-user analytics, if ever needed, must be a separate audited endpoint.
...
### Deferred
- ChatWidget decomposition ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â tracked for its own focused PR.

---

## [0.27.0] - 2026-06-15

Production-refactor batch (Phases 3 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 4): frontend entitlement gating + backend hardening.
Monetization enforcement remains **disabled** by default ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â no UX change.
...
### Changed / Fixed
- `SqlGuard` now rejects subqueries/CTEs, guaranteeing the fallback NL ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â SQL path is a single flat
 SELECT ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â closes the `appendUserFilter` user-scoping bypass flagged in the AI-path security review.
...
Production-refactor batch (Phases 0 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 2). All changes are backwards compatible and additive;
...
- Safe AI analytics query pipeline (`ai.query`): the LLM emits a structured, validated query intent
 ...
 to the existing SqlGuard NL ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â SQL path when no valid intent is produced, so no analytics is lost.
...
- `EntitlementService` (`canAccessFeature`/`requireFeatureAccess`) as the single access authority;
 ...
 `/ai/insights/*`, `/expenses/export`. `FeatureLockedException` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â HTTP 402.
...
### Added
- Chatbot CRUD actions via LLM function-calling (`POST /ai/chat`) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â create/delete expenses, budgets, goals, and recurring items in plain language
...
### Added
- `RECURRING_DUE` alert type ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â generates reminders when a recurring expense is due within 3 days
- `NotificationBell` component in Navbar ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Bell icon with unread count badge, replaces text "Alerts" link
...
### Changed
- `spring.jpa.hibernate.ddl-auto` switched from `create-drop` to `validate` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â schema now managed exclusively by Flyway migrations
...
### Fixed
- Dashboard cards (BudgetOverview, DailyTrend, TopExpenses, AlertsCard, GoalProgress, UpcomingBills) now re-fetch whenever any related data changes ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â not just on mount
...
### Added
- `GET /expenses/report/daily?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns daily spending totals for all days in a month (missing days = 0); enables day-by-day trend analysis
- `GET /expenses/report/top?month=YYYY-MM&limit=5` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns top N expenses by amount descending for a given month
- Daily Trend card on Dashboard ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â bar chart showing per-day spending with recharts BarChart
- Top Expenses card on Dashboard ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ranked list of highest single expenses for the current month
...
### Added
- `expenseType` field on `Expense` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â enum (PERSONAL/BUSINESS); defaults to PERSONAL
- `reimbursable` boolean field on `Expense` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â tracks whether expense can be reimbursed; defaults to false
...
### Added
- `Alert` entity ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â stores budget warnings, budget-exceeded alerts, and spending-spike nudges per user per month; tracks read/dismissed state
- `GET /alerts?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â auto-generates alerts from live budget + expense data (idempotent upsert); returns non-dismissed alerts sorted by severity
- `PATCH /alerts/{id}/read` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â mark alert as read; `PATCH /alerts/{id}/dismiss` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â dismiss alert (hides from future responses); `DELETE /alerts/{id}` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â hard delete
- Alert types: `BUDGET_WARNING` ( ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 80% of budget spent), `BUDGET_EXCEEDED` ( ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 100%), `SPENDING_SPIKE` (current month > previous month ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 1.5)
- Alerts page (`/alerts`) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â month picker, severity badges (CRITICAL/WARNING/INFO), mark-read and dismiss per alert
- Dashboard: **Spending Alerts** card ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â shows up to 4 active alerts; mark-read button inline
...
### Added
- `SavingsGoal` entity ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â named goal with `targetAmount`, `savedAmount`, optional `targetDate`, `paused` flag; per-user scoped
- Status derivation: `PAUSED` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â `COMPLETED` (saved ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â target) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â `ON_TRACK` / `BEHIND` (linear interpolation against elapsed time fraction when `targetDate` present; `ON_TRACK` when no deadline)
- `GET /goals`, `POST /goals`, `GET /goals/{id}`, `PUT /goals/{id}`, `DELETE /goals/{id}` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â full CRUD for savings goals
- Goals page (`/goals`) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â card grid with progress bar, status badge, add/edit/delete modals
- Dashboard: **Goal Progress** card ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â shows up to 4 active (non-PAUSED, non-COMPLETED) goals sorted by target date; links to Goals page
...
### Added
- `GET /expenses/export?from=&to=` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â exports all expenses matching the current date filter as a CSV file; both params optional; respects per-user scoping
- CSV columns: `Date` (yyyy-MM-dd HH:mm), `Description`, `Category`, `Amount` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â roundtrippable with the existing CSV import format
- Expenses page: **Export CSV** button in the filter toolbar; passes active `from`/`to` filter so "what you see is what you download"; shows "Exporting ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â " spinner while download is in progress
...
### Added
- Recurring Bills page (`/recurring`) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â CRUD for expenses that repeat monthly, weekly, or yearly; YEARLY bills scoped to a specific month via `monthOfYear` field; monthly day capped at 28 for MONTHLY frequency to fire every month including February; YEARLY respects target month's actual length
- `GET /recurring/upcoming?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns all bills due in the given month with ISO due dates; weekly bills expand to individual occurrences
- Dashboard: Upcoming Bills card ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â shows up to 5 upcoming bills for the current month; links to Recurring page
- Recurring nav link in sidebar
- `CategoryCombobox` shared component ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â type-to-filter + select from existing categories; "Create" affordance when typed name has no match; `allowCreate` prop; used in both Budget and Recurring forms
...
- `DELETE /budgets?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â deletes all budgets for the authenticated user in the given month
- `DELETE /recurring` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â deletes all recurring expenses for the authenticated user
...
- Demo seed data: 54 synthetic expenses spanning Jan ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Jun 2026 across all 13 predefined categories; 5 sample budgets for June 2026; 6 recurring expenses seeded on first startup when `GASTOS_SEED_SAMPLE_DATA=true`
...
### Added
- `GET /ai/insights/top-category?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns the top spending category, total, and percentage of month total for the given month
- `GET /ai/insights/month-summary?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns an AI-generated natural-language spending summary for the month
- `GET /ai/insights/recommendations?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns 2 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 3 AI-generated actionable spending recommendations based on category and MoM data
- Dashboard: AI Insights card ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â displays top category, month summary paragraph, and recommendations list; renders below Budget Overview; supports dark mode and animated skeleton loading state
- `ExpenseService.categoryReportForMonth()` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â month-scoped category aggregation used by all three insight endpoints
...
### Added
- `GET /expenses?from=YYYY-MM-DD&to=YYYY-MM-DD` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â date-range filtering on expense list; either param is optional; admin users see all matching expenses, regular users see only their own
- `GET /expenses/report/monthly-comparison?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns current month total, previous month total, and percentage change; `changePercent` is null when no previous data exists
- Dashboard: Monthly Trend bar chart ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â visualises up to 12 months of spending using recharts `BarChart`
- Dashboard: Spending Trend card ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â side-by-side "This month / Last month" amounts with a plain-English change sentence ("You spent 11.8% more than May 2026") colour-coded green/red
...
- `ui-ux-reviewer` agent ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â reviews dashboard and table UI/UX decisions against data-viz and financial-app best practices; sourced from TanStack Table, Pencil & Paper, UXPin
- `resource-finder` agent ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â searches and ranks libraries/tools by adoption, community consensus, security posture, and real-user satisfaction before any new dependency is introduced
...
### Added
- Budgets feature ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â set monthly spending limits per category via `POST /budgets`; full CRUD (`GET`, `PUT`, `DELETE`) with per-user ownership enforcement
- `GET /budgets/summary?month=YYYY-MM` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â returns per-category spent vs budgeted, remaining, percent used, and status (`ON_TRACK` / `WARNING` at 80% / `OVER_BUDGET` at 100%)
...
- Budget page (`/budget`) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â month picker, budget list with inline add/edit/delete modal, category dropdown
- Budget Overview card on Dashboard ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â headline safe-to-spend, daily allowance, status-coloured progress bars per category

### Changed
- Semantic versioning rules updated: `BREAKING CHANGE` / `feat!:` / `fix!:` now always bumps MAJOR (e.g. `0.9.0 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 1.0.0`); pre-1.0 exception removed to follow Conventional Commits spec
...
### Changed
- Category lookups and creation are now case-insensitive ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â `"food"`, `"Food"`, and `"FOOD"` resolve to the same category; duplicate creation via different casings is rejected
...
### Added
- `POST /expenses/parse` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â natural-language text parse endpoint; accepts free-form text (e.g. "spent 250 on lunch") and returns a draft expense with amount, category, date, description, and confidence level without saving
...
- ChatWidget: expense-logging intent detection ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â messages with spending keywords (English + Filipino), currency indicators, or a numeric amount without a query phrase are routed to `POST /expenses/parse`
- ChatWidget: draft expense card ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â shows amount, category, date, and description with a "Save expense" button; fires `gastosai:expense-created` on save so Expenses and Dashboard lists refresh immediately
...
### Fixed
- Expenses list and Dashboard recent expenses now sorted by date DESC ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â newly added expenses appear at the top
...
### Added
- Avatar color picker in Settings ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â users can choose from 6 gradient presets; selection is persisted and reflected immediately in the Navbar avatar and Settings header
- Category icon picker ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â add/edit category modal now shows a 24-icon grid; selected icon is stored on the backend and displayed on the category card; falls back to name-based icon mapping if none set
- Email editing in Settings ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â email field is now editable; backend validates uniqueness and issues a fresh JWT so the session stays valid after an email change

### Fixed
- Dark mode text visibility in ChatWidget ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â suggestion chips and accent text in all 3 modes (Plain, Pro, Gen Z) now have proper dark-mode colour variants
...
- Navbar profile link replaced with an avatar button showing user initials in a circle ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â clearly clickable with hover state and active highlight; name label visible on sm+ screens
...
### Added
- Full Docker stack via `docker compose --profile app up -d --build` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â backend and frontend now containerised alongside the existing DB service
...
- `frontend/Dockerfile` multi-stage build: Node 22 Alpine ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â nginx Alpine (~20 MB runtime image)
...
- Pre-PR checklist: added mandatory runtime execution-testing rule ( ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â 90% of touched paths), infrastructure breaking-change rule, and PS encoding rule
...
- Added `ai/skills/shared/pre-pr-checklist.md` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â mandatory lint/build/test gate before every PR
...
### Fixed
- CI `Permission denied` error on `./mvnw` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â set executable bit in git index
...
- Admin users bypass per-user expense scoping ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â sees all users' data in expenses, reports, and AI queries
...
- Per-user expense data isolation ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â each user sees only their own expenses
