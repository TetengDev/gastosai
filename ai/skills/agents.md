...
| E2E + capture + Slack report | "end-to-end", "screenshot/recording", "send to Slack", "verify before merge" | `e2e-release-verification.md` → `qa-testing.md` | `qa-e2e-reporter` (after `qa-engineer`/`security-auditor`) |
...
| `qa-e2e-reporter` | Run the Playwright E2E suite against the running app, collect screenshots + video, send artifacts to Slack; run/report only | Sonnet |
